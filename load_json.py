...
from config.postgree_db import PostgreSQL

# IMPORT SQL
from sql.query import create_table_dim, create_table_fact


with open ('credential_db.json', "r") as cred:
        credential = json.load(cred)


def insert_raw_data():
  mysql_auth = MySQL(credential['mysql_db'])
...
def create_star_schema(schema):
  postgre_auth = PostgreSQL(credential['postgresql_db'])
